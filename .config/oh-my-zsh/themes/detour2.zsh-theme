...
function virtual_env_prompt_info() {
  if [[ -n $VIRTUAL_ENV ]]; then
    # Extract the last name from the virtual environment path
    venv_name=$(basename $VIRTUAL_ENV)
    echo "%{$fg_bold[green]%}(venv:$venv_name) "
  fi
}
...
# Modified PROMPT that includes Conda and venv information
PROMPT='$(conda_env_prompt_info)$(virtual_env_prompt_info)%{$fg_no_bold[cyan]%}%n%{$fg_no_bold[magenta]%}$(ssh_hostname)•%{$fg_no_bold[green]%}%3~$(git_prompt_info)%{$reset_color%}» '
